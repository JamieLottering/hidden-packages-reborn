...
local function registerNativeSettingsMaps(nativeSettings, nativeSettingsRoot)
	...
	nativeSettings.addSelectorString(
      category, "Map", "Maps are stored in '.../mods/Hidden Packages/Maps''. If set to None the mod is disabled.", nsMapsDisplayNames, nsCurrentMap, nsDefaultMap, function(value)
...
local function registerNativeSettingsSonar(nativeSettings, nativeSettingsRoot)
	...
	nativeSettings.addSwitch(
      category, "Sonar", "Play a sound when near a package in increasing frequency the closer you get to it", MOD_SETTINGS.SonarEnabled, false, function(state)
		MOD_SETTINGS.SonarEnabled = state
		saveSettings()
	end)
	nativeSettings.addRangeInt(
      category, "Range", "Sonar starts working when this close to a package", 50, 250, 25, MOD_SETTINGS.SonarRange, 125, function(value)
		MOD_SETTINGS.SonarRange = value
		saveSettings()
	end)
	nativeSettings.addRangeFloat(
      category, "Minimum Interval", "Sonar will wait atleast this long before playing a sound again. Value is in seconds.", 0.0, 10.0, 0.5, "%.1f", MOD_SETTINGS.SonarMinimumDelay, 0.0, function(value)
...
local function registerNativeSettingsScanner(nativeSettings, nativeSettingsRoot)
	...
	nativeSettings.addSwitch(
      category, "Scanner Marker", "Nearest package will be marked by using the scanner", MOD_SETTINGS.ScannerEnabled, false, function(state)
		MOD_SETTINGS.ScannerEnabled = state
		saveSettings()
	end)
	nativeSettings.addRangeInt(
      category, "Sticky Markers", "Keep up to this many packages marked. Markers will disappear after closing the scanner if set to 0.", 0, 10, 1, MOD_SETTINGS.StickyMarkers, 0, function(value)
...
local function registerNativeSettingsRewards(nativeSettings, nativeSettingsRoot)
	...
	nativeSettings.addRangeInt(
      category, "Money", "Collecting a package rewards you this much money", 0, 5000, 100, MOD_SETTINGS.MoneyPerPackage, 1000, function(value)
		MOD_SETTINGS.MoneyPerPackage = value
		saveSettings()
	end)
	nativeSettings.addRangeInt(
      category, "XP", "Collecting a package rewards you this much XP", 0, 300, 10, MOD_SETTINGS.ExpPerPackage, 100, function(value)
		MOD_SETTINGS.ExpPerPackage = value
		saveSettings()
	end)
	nativeSettings.addRangeInt(
      category, "Street Cred", "Collecting a package rewards you this much Street Cred", 0, 300, 10, MOD_SETTINGS.StreetcredPerPackage, 100, function(value)
		MOD_SETTINGS.StreetcredPerPackage = value
		saveSettings()
	end)
	nativeSettings.addRangeFloat(
      category, "Reward Multiplier", "Multiply rewards (except random items) by how many packages you've collected and this.\n(eg. 1.0 means 5th package = 5x the rewards. 0.0 disables it (every package will give you 1x))", 0.0, 2.0, 0.1, "%.1f", MOD_SETTINGS.PackageMultiplier, 1.0, function(value)
...
local function registerNativeSettings()
	...
	nativeSettings.addSubcategory(
      nativeSettingsRoot .. "/Version", HiddenPackagesMetadata.title .. " version " .. HiddenPackagesMetadata.version .. " (" .. HiddenPackagesMetadata.date .. ")")
